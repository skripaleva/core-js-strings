...
function removeFirstOccurrences(str, value) {
  ...
  return (
    (index !== -1 && str.slice(0, index) + str.slice(index + value.length)) ||
    str
  );
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (Array.from(vowels).includes(str[i])) {
      res += 1;
    }
  }
  return res;
...
function findLongestWord(sentence) {
  const sortArr = sentence.split(' ').sort((a, b) => {
 ...
 *
 * @param {string} str - The input string.
 * @return {void} - The string where each word is reversed.
 *
 * @example:
...
function encodeToRot13(str) {
  ...
  return str
    .replace(/[a-z]/g, (letter) => alphabet[alphabet.indexOf(letter) + 13])
    .replace(/[A-Z]/g, (letter) => upper[upper.indexOf(letter) + 13]);
}
